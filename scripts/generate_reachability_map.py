...
MAP_SIZE = [[-1, 2], [-1, 2], [-1, 1]]  # m
VOXEL_SIZE = 0.01  # m
ANG_STEP_SIZE = math.radians(10)  # rad
...
class ReachabilityMap:
    ...
    def __init__(self, robot_name, joint_group_name):
        ...
        # self.endeffector_name = joint_group.eef_name
    ...
    def generate_reachability_map(self):
        ...
        def try_configurations_recursivly(joint_names, i):
            if len(joint_names) == 0:
                ...
                # i_x, i_y, i_z = rmap_entry_from_xyz(pose.position.x, pose.position.y, pose.position.z)
                self.rmap[i_x, i_y, i_z] = self.rmap[i_x, i_y, i_z] + 1
                # print(str(self.current_positions))
                self.pose_counter += 1
                if self.pose_counter % 1000 == 0:
                    print(f"Computed {self.pose_counter}/{self.poses_to_compute}")
                return
            for j in range(self.steps_per_joint[i]):
                self.current_positions[i] = j * ANG_STEP_SIZE
            ...
            for j in range(self.rmap.shape[1]):
                for k in range(self.rmap.shape[2]):
                    ...
                    if val > 0:
                        ...
                        marker.color.g = g
                        marker.color.b = 0.0
                        marker.color.a = 0.5  # todo make parameter
...
def main():
    ...
    parser.add_argument("--joint-group-name", type=str,
                        help="Name of the joint group", default="endo")
    ...
    x = np.arange(rmap.rmap.shape[0])
    y = np.arange(rmap.rmap.shape[1])
    xx, yy = np.meshgrid(x, y)
    ax.plot_surface(xx, yy, rmap.rmap[:, :, 5], cmap='viridis', alpha=0.8)
